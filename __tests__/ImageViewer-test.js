...
test('ImageViewer allows for manifests to be set in the query string', () => {
  ...
  expect(iframe.find('iframe[src="/src/mirador#?manifest=qsmanifest&c=&m=&s=&cv=&xywh="]').exists()).toEqual(true);
})

test('ImageViewer Renders a default Iframe with default data', () => {
  const iframe = shallow(<ImageViewer location={ location } /> );
  expect(iframe.find('iframe').html()).toEqual('<iframe id="viewerFrame" width="800" height="600" src="/src/mirador#?manifest=https://presentation-iiif.library.nd.edu/002209668/manifest&amp;c=&amp;m=&amp;s=&amp;cv=&amp;xywh="></iframe>');
});
...
test('ImageViewer allows the manifest to be set', () => {
  ...
  expect(iframe.find('iframe[src="/src/mirador#?manifest=manifest&c=&m=&s=&cv=&xywh="]').exists()).toEqual(true);
});
...
test('ImageViewer allows opaque uv property c to be set', () => {
  ...
  expect(iframe.find('iframe[src="/src/mirador#?manifest=manifest&c=50&m=&s=&cv=&xywh="]').exists()).toEqual(true);
});
...
test('ImageViewer allows opaque uv property s to be set', () => {
  ...
  expect(iframe.find('iframe[src="/src/mirador#?manifest=manifest&c=&m=&s=50&cv=&xywh="]').exists()).toEqual(true);
});
...
test('ImageViewer allows opaque uv property m to be set', () => {
  ...
  expect(iframe.find('iframe[src="/src/mirador#?manifest=manifest&c=&m=50&s=&cv=&xywh="]').exists()).toEqual(true);
});
...
test('ImageViewer allows opaque uv property cv to be set', () => {
  ...
  expect(iframe.find('iframe[src="/src/mirador#?manifest=manifest&c=&m=&s=&cv=50&xywh="]').exists()).toEqual(true);
});
...
test('ImageViewer allows opaque uv property xywh to be set', () => {
  ...
  expect(iframe.find('iframe[src="/src/mirador#?manifest=manifest&c=&m=&s=&cv=&xywh=50"]').exists()).toEqual(true)
});
